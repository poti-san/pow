...
class PowerKnownSchemeFactory:
    """既知の電力スキームを作成するファクトリクラスです。"""

    @staticmethod
    def create_maxpowersavings() -> PowerScheme:
        GUID_MAX_POWER_SAVINGS = Guid.from_define(
            0xA1841308, 0x3541, 0x4FAB, 0xBC, 0x81, 0xF7, 0x15, 0x56, 0xF2, 0x0B, 0x4A
        )
        return PowerScheme(GUID_MAX_POWER_SAVINGS)

    @staticmethod
    def create_minpowersavings() -> PowerScheme:
        GUID_MIN_POWER_SAVINGS = Guid.from_define(
            0x8C5E7FDA, 0xE8BF, 0x4A96, 0x9A, 0x85, 0xA6, 0xE2, 0x3A, 0x8C, 0x63, 0x5C
        )
        return PowerScheme(GUID_MIN_POWER_SAVINGS)

    @staticmethod
    def create_typicalpowersavings() -> PowerScheme:
        GUID_TYPICAL_POWER_SAVINGS = Guid.from_define(
            0x381B4222, 0xF694, 0x41F0, 0x96, 0x85, 0xFF, 0x5B, 0xB2, 0x60, 0xDF, 0x2E
        )
        return PowerScheme(GUID_TYPICAL_POWER_SAVINGS)

    @staticmethod
    def create_allpowerschemes() -> PowerScheme:
        ALL_POWERSCHEMES_GUID = Guid.from_define(
            0x68A1E95E, 0x13EA, 0x41E1, 0x80, 0x11, 0x0C, 0x49, 0x6C, 0xA4, 0x90, 0xB0
        )
        return PowerScheme(ALL_POWERSCHEMES_GUID)

    @staticmethod
    def create_personality() -> PowerScheme:
        GUID_POWERSCHEME_PERSONALITY = Guid.from_define(
            0x245D8541, 0x3943, 0x4422, 0xB0, 0x25, 0x13, 0xA7, 0x84, 0xF6, 0x79, 0xB7
        )
        return PowerScheme(GUID_POWERSCHEME_PERSONALITY)

    @staticmethod
    def create_activescheme() -> PowerScheme:
        GUID_ACTIVE_POWERSCHEME = Guid.from_define(
            0x31F9F286, 0x5084, 0x42FE, 0xB7, 0x20, 0x2B, 0x02, 0x64, 0x99, 0x37, 0x63
        )
        return PowerScheme(GUID_ACTIVE_POWERSCHEME)
